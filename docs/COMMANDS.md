...
Host lifecycle hook for task start. It runs prepare and records usage automatically.
...
Host lifecycle hook for task completion, failure, or correction. It runs reflect and records usage automatically. Add `--write-candidate` to write safe low/medium-risk candidates to inbox.
