...
Usage is appended to `memory/evolution-os/runtime/usage-log.jsonl`. The report highlights effective lessons and lessons that are repeatedly prepared but not applied, supporting later promotion, compression, or forgetting. By default, a lesson must be prepared at least 3 times with 0 applications before cleanup is suggested; a lesson must be applied at least 3 times before promotion is suggested. Configure this via `runtime.stalePreparedOnlyThreshold` and `runtime.effectivePromotionThreshold` in `memory/evolution-os/config.json`.

To turn repeatedly applied lessons into reviewable promotion candidates:

```bash
node tools/evolution-review.js --usage-report --suggest-promotion-candidates
```

This writes only inbox candidates. It does not modify trigger files, skills, or core memory.
